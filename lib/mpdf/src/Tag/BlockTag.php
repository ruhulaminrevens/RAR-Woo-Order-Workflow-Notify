...
use RarWowVendor\Mpdf\Conversion\DecToAlpha;
use RarWowVendor\Mpdf\Conversion\DecToCjk;
use RarWowVendor\Mpdf\Conversion\DecToHebrew;
use RarWowVendor\Mpdf\Conversion\DecToOther;
use RarWowVendor\Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \RarWowVendor\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \RarWowVendor\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
