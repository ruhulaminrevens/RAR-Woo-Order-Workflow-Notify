 ...
 */
class Code11 extends \RarWowVendor\Mpdf\Barcode\AbstractBarcode implements \RarWowVendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio)
	{
		...
		for ($i = 0; $i < $len; ++$i) {
			...
			if (!isset($chr[$code[$i]])) {
				throw new \RarWowVendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE11 barcode value "%s"', $code[$i], $code));
			}
