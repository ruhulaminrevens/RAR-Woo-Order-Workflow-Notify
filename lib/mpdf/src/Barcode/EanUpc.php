 ...
 */
class EanUpc extends \RarWowVendor\Mpdf\Barcode\AbstractBarcode implements \RarWowVendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length)
	{
		if (preg_match('/[\D]+/', $code)) {
			throw new \RarWowVendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN UPC barcode value "%s"', $code));
		}
		...
		if ($upce) {
			...
			if ($invalidUpce) {
				throw new \RarWowVendor\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
			}
		}
