...
use RarWowVendor\Mpdf\File\LocalContentLoaderInterface;
use RarWowVendor\Mpdf\File\StreamWrapperChecker;
use RarWowVendor\Mpdf\Http\ClientInterface;
use RarWowVendor\Mpdf\Log\Context as LogContext;
use RarWowVendor\Mpdf\PsrHttpMessageShim\Request;
use RarWowVendor\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use RarWowVendor\Psr\Log\LoggerInterface;

class AssetFetcher implements \RarWowVendor\Psr\Log\LoggerAwareInterface, \RarWowVendor\Mpdf\AssetFetcherInterface
{
	...
	public function fetchDataFromPath($path, $originalSrc = null)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \RarWowVendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
	...
	public function fetchRemoteContent($path)
	{
		...
		try {
			...
			/** @var \RarWowVendor\Mpdf\PsrHttpMessageShim\Response $response */
			...
			if (!str_starts_with((string) $response->getStatusCode(), '2')) {
				...
				if ($this->mpdf->debug) {
					throw new \RarWowVendor\Mpdf\MpdfException($message);
				}
			...
			if ($this->mpdf->debug) {
				throw new \RarWowVendor\Mpdf\MpdfException($message, 0, E_ERROR, null, null, $e);
			}
